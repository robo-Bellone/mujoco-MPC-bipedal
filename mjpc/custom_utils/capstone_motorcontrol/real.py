          ...
          with TMotorManager_mit_can(motor_type=Type['others'] , motor_ID= Motor['R_knee'], CSV_file='R_knee.csv') as R_knee :
           with TMotorManager_mit_can(motor_type=Type['ankle'] , motor_ID= Motor['L_ankle'], CSV_file='L_ankle.csv') as L_ankle :
            with TMotorManager_mit_can(motor_type=Type['ankle'] , motor_ID= Motor['R_ankle'],CSV_file='R_ankle.csv') as R_ankle :         
                ...
                try:
                    time.sleep(1)  
                    loop = SoftRealtimeLoop(dt = 0.0005, report= True, fade=0.0)
                    ...
                    for t in loop :
                        ...
                        # #Update(R_waist=R_waist,R_hip=R_hip,R_knee=R_knee,R_ankle=R_ankle,L_waist=L_waist,L_hip=L_hip,L_knee=L_knee,L_ankle=L_ankle)
                        impedance_mode(L_ankle,command[Motor['L_ankle']-2]['position'],40,5)
                        impedance_mode(R_waist,command[Motor['R_waist']-2]['position'],15,1)
                        impedance_mode(L_waist,command[Motor['L_waist']-2]['position'],15,1)
                        impedance_mode(R_hip,command[Motor['R_hip']-2]['position'],15,1)
                        impedance_mode(L_hip,command[Motor['L_hip']-2]['position'],15,1)
                        impedance_mode(R_knee,command[Motor['R_knee']-2]['position'],15,1)
                        impedance_mode(L_knee,command[Motor['L_knee']-2]['position'],15,1)
                        impedance_mode(R_ankle,command[Motor['R_ankle']-2]['position'],40,5)
