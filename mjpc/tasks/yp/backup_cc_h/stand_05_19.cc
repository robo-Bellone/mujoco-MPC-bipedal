...
//     Residual(7): position - goal position + heading * goal position
// -------------------------------------------
void YP::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  double left_fk_cost = ((pi - data->qpos[9])/2 * data->qpos[7]) - plane_margin_param;
  double right_fk_cost = ((pi - data->qpos[13])/2 * data->qpos[11]) - plane_margin_param;
...
void YP::TransitionLocked(mjModel* model, mjData* d) {
  ...
  // d->qpos[7] = -mc.data->feedback[1].position;  // 라디안 단위 //hip r 2
  // d->qpos[8] = -mc.data->feedback[3].position;  // 라디안 단위 //thigh r 3
  // d->qpos[9] = -mc.data->feedback[5].position;  // 라디안 단위 //shin r 4
  // d->qpos[10] = -mc.data->feedback[7].position;  // 라디안 단위 //foot r 5
  // d->qpos[11] = mc.data->feedback[0].position;  // 라디안 단위 //hip r 6
  // d->qpos[12] = mc.data->feedback[4].position;  // 라디안 단위 //thigh r 7
  // d->qpos[13] = mc.data->feedback[2].position;  // 라디안 단위 //shin r 8 
  // d->qpos[14] = -mc.data->feedback[6].position;  // 라디안 단위 //foot r 9
