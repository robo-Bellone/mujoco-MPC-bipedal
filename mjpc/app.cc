...
// Real 2 Sim feedforward IPC tool
  ...
  while (!sim.exitrequest.load()) {
...

    {
      ...
      if (m) {  // run only if model is present
        // mc.readData();
        // d->qpos[7] = (d->qpos[7] + mc.data->feedback[0].position)/2;  // 라디안 단위 //hip r 2
        // d->qpos[8] = (d->qpos[8] + mc.data->feedback[1].position)/2;  // 라디안 단위 //thigh r 3
        // d->qpos[9] = (d->qpos[9] + mc.data->feedback[2].position)/2;  // 라디안 단위 //shin r 4
        // d->qpos[10] = (d->qpos[10] + mc.data->feedback[3].position)/2;  // 라디안 단위 //foot r 5
        // d->qpos[11] = (d->qpos[11] + mc.data->feedback[4].position)/2;  // 라디안 단위 //hip r 6
        // d->qpos[12] = (d->qpos[12] + mc.data->feedback[5].position)/2;  // 라디안 단위 //thigh r 7
        // d->qpos[13] = (d->qpos[13] + mc.data->feedback[6].position)/2;  // 라디안 단위 //shin r 8 
        // d->qpos[14] = (d->qpos[14] + mc.data->feedback[7].position)/2;  // 라디안 단위 //foot r 9
